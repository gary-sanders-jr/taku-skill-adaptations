---
name: offsite-objective-evidence-audit
description: "Use supplied, sanitized material to audit supplied offsite objectives and assumptions in a private note without booking, inviting, scheduling, or facilitating an event; returns a private, unapplied draft only."
---

# Offsite Objective Evidence Audit

Use this skill only with sanitized material the user supplies in the current conversation. Its purpose is to audit supplied offsite objectives and assumptions in a private note without booking, inviting, scheduling, or facilitating an event.

## Produce

Record the supplied objective, participant labels, intended outcomes, evidence, constraints, dependencies, unresolved decisions, risks, owner questions, and NOT PLANNED status. Preserve qualifiers, distinguish supplied facts from assumptions, and label missing support as `unknown`. Keep the result private and unapplied.
