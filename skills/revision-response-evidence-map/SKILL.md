---
name: revision-response-evidence-map
description: "Use supplied, sanitized material to map supplied review comments to a private revision-response evidence plan without editing, submitting, or claiming resolution; returns a private, unapplied draft only."
---

# Revision Response Evidence Map

Use this skill only with sanitized material the user supplies in the current conversation. Its purpose is to map supplied review comments to a private revision-response evidence plan without editing, submitting, or claiming resolution.

## Produce

Record each supplied comment, interpreted request, supporting excerpt, proposed response, proposed revision location, unresolved ambiguity, dependency, owner question, and NOT REVISED status. Preserve qualifiers, distinguish supplied facts from assumptions, and label missing support as `unknown`. Keep the result private and unapplied.
